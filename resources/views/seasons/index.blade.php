...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-header">
            <div>
                <h3 class="card-title">{{ __('Season List') }}</h3>
                <p class="card-description">{{ __('Manajemen musim/season (2026, 2027, dsb) per academy.') }}</p>
            </div>
        ...
        <div class="border-b border-gray-100 p-4 dark:border-gray-800">
            <x-table.tabs route="seasons.index" :active="$filters['status'] ?? ''" :tabs="[
        ...
        <x-table.toolbar route="seasons.index" :filters="$filters" placeholder="{{ __('Cari nama season...') }}">
                ...
                <tbody class="table-body">
                    ...
                    @forelse ($seasons as $season)
                        ...
                        <tr class="table-row">
                            ...
                            <td class="table-cell">
                                <div>
                                    <span class="table-title">{{ $season->name }}</span>
                            ...
                            @if ($isSuperAdmin)
                                <td class="table-cell">
                                    <span class="badge badge-secondary">{{ $season->academy->name }}</span>
                                </td>
                            @endif
                            ...
                            <td class="table-cell text-right">
                                <div class="table-action">
                                    ...
                                    @can('season.delete')
                                        <x-button.delete :action="route('seasons.destroy', $season)"
                                            :name="$season->name" :disabled="$season->teams_count > 0"
                                            reason="{{ __('Season masih digunakan oleh tim, tidak dapat dihapus.') }}" />
                                    @endcan
                        ...
                        <tr>
                            <td colspan="{{ $isSuperAdmin ? 4 : 3 }}" class="table-empty">
                                <div class="empty-state">
                                    ...
                                    <h4 class="empty-title">{{ __('Belum ada Season') }}</h4>
                                    <p class="empty-description">{{ __('Tambahkan season pertama.') }}</p>

                                    @can('season.create')
                                        <a href="{{ route('seasons.create') }}" class="empty-link">{{ __('Tambah Season') }}</a>
                                    @endcan
            ...
            @forelse ($seasons as $season)
                <div class="table-card">
                    <div class="table-card-header">
                        <div class="min-w-0">
                            <span class="table-title truncate">{{ $season->name }}</span>
                            <span class="table-subtitle">
                                @if ($season->start_date || $season->end_date)
                                    {{ $season->start_date?->format('d M Y') }} - {{ $season->end_date?->format('d M Y') }}
                                @else
                        ...
                        @if ($isSuperAdmin)
                            <span class="badge badge-secondary shrink-0">{{ $season->academy->name }}</span>
                        @endif
                    </div>
                    ...
                    <div class="table-card-actions">
                        @can('season.update')
                            <a href="{{ route('seasons.edit', $season) }}" class="btn-icon btn-icon-warning"
                                title="{{ __('Edit') }}">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                                    <path d="M13.75 2.5L17.5 6.25L6.25 17.5H2.5V13.75L13.75 2.5Z"
                                        stroke="currentColor" stroke-width="1.5" />
